<script lang="ts">
  ...
  let groupOptions = $derived(
    buildGroupOptions(
      groups.filter(
        (g) =>
          g.can_create_monitor ||
          (!!editingMonitor && g.id === editingMonitor.group_id),
      ),
    ),
  );
  let topLevelOptionAllowed = $derived(
    auth.user?.is_admin === true ||
      auth.user?.can_create_top_level_monitors === true ||
      (!!editingMonitor && editingMonitor.group_id == null),
  );
  ...
  $effect(() => {
    monitorGroupsApi
      ...
      .then((list) => {
        groups = list;
        // Non-admin creators cannot place monitors at top level. Once the
        // creatable folders load, default the picker to the first allowed one
        // so submit does not fail on an empty group_id that is not offered.
        if (
          !editingMonitor &&
          auth.user?.is_admin !== true &&
          selectedGroupId === ""
        ) {
          const first = list.find((g) => g.can_create_monitor);
          if (first) selectedGroupId = String(first.id);
        }
      })
      .catch(() => {
        groups = [];
  ...
  $effect(() => {
    ...
    Promise.all([
      escalationApi.list(),
      monitorId
        ? escalationApi.getForMonitor(monitorId)
        : Promise.resolve(null),
    ])
      .then(([list, assignment]) => {
        escalationPolicies = list;
        escalationBaselineId =
          assignment && assignment.policy_id
            ? String(assignment.policy_id)
            : "";
  ...
  let visibleFields = $derived(
    (monitorTypeConfig[selectedType]?.fields || []).filter((f) =>
      isFieldVisible(f),
    ),
  );
  ...
  async function handleSubmit() {
    ...
    if (
      ...
      toast.error(m.monitor_form_group_required());
      return;
    }
    ...
    if (selectedType === "http") {
      ...
      if (method === "basic") {
        if (
          !need("auth_username", "Username") ||
          !need("auth_password", "Password")
        )
          return;
  ...
  async function syncEscalationAssignment(monitorId: number) {
    ...
    try {
      await escalationApi.setForMonitor(
        monitorId,
        escalationPolicyId === "" ? 0 : Number(escalationPolicyId),
      );
...
{#if open}
  ...
  >
    ...
    >
      ...
      >
        <h3
          id="monitor-form-title"
          class="text-lg font-semibold tracking-tight"
        >
          {monitor ? m.monitor_form_edit_title() : m.monitors_create()}
        </h3>
        <button
          type="button"
          onclick={close}
          ...
          aria-label={m.btn_close()}><X class="h-5 w-5" /></button
        >
      </div>
      ...
      <div class="mt-6 space-y-6">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label for="monitor-name" class="text-sm font-medium"
              >{m.monitor_form_name_label()}</label
            >
            <input
              id="monitor-name"
              type="text"
          ...
          <div>
            <label for="monitor-type" class="text-sm font-medium"
              >{m.monitor_form_type_label()}</label
            >
            <div class="mt-1">
              <Select
                id="monitor-type"
          ...
          <div>
            <label for="monitor-interval" class="text-sm font-medium"
              >{m.monitor_form_interval_label()}</label
            >
            <input
              id="monitor-interval"
              type="number"
          ...
          <div>
            <label for="monitor-timeout" class="text-sm font-medium"
              >{m.monitor_form_timeout_label()}</label
            >
            <input
              id="monitor-timeout"
              type="number"
        ...
        <div class="border-t border-border pt-6">
          <h4 class="mb-3 text-sm font-semibold text-muted-foreground">
            {m.monitor_form_type_config_heading()}
          </h4>
              ...
              <div class="flex gap-2">
                ...
                <div class="min-w-0 space-y-2">
                  <p class="font-medium text-foreground">
                    {m.docker_monitor_warning_title()}
                  </p>
        ...
        <div class="border-t border-border pt-6">
          <h4 class="mb-3 text-sm font-semibold text-muted-foreground">
            {m.monitor_form_advanced_heading()}
          </h4>
          <div class="grid grid-cols-1 gap-x-4 gap-y-4 md:grid-cols-2">
            <div class="md:col-span-2 space-y-3">
              <div class="flex items-start gap-2">
                <input
                  id="monitor-inherit-owner"
                  type="checkbox"
                  class="mt-1 h-4 w-4 rounded border-border"
                  checked={formData.inheritGroupOwner}
                  disabled={selectedGroupId === ""}
                  onchange={(e) =>
                    (formData.inheritGroupOwner = (
                ...
                <div class="min-w-0">
                  ...
                  <p class="mt-0.5 text-xs text-muted-foreground">
                    {selectedGroupId === ""
                      ? m.monitor_form_inherit_owner_needs_group()
                      : m.monitor_form_inherit_owner_help()}
                  </p>
                </div>
              </div>
              <div>
                <label class="text-sm font-medium" for="monitor-owner"
                  >{m.monitor_form_owner_label()}</label
                >
                <input
                  id="monitor-owner"
                  type="text"
                ...
                <p class="mt-1 text-xs text-muted-foreground">
                  {formData.inheritGroupOwner && selectedGroupId !== ""
                    ? m.monitor_form_owner_inherited_help()
                    : m.monitor_form_owner_help()}
                </p>
              </div>
            </div>

            <div class="md:col-span-2">
              <label class="text-sm font-medium" for="monitor-description"
                >{m.monitor_form_description_label()}</label
              >
              <textarea
            ...
            <div>
              <label class="text-sm font-medium" for="monitor-retry-interval"
                >{m.monitor_form_retry_interval_label()}</label
              >
              <input
            ...
            <div>
              <label class="text-sm font-medium" for="monitor-max-retries"
                >{m.monitor_form_max_retries_label()}</label
              >
              <input
              ...
              />
              <p class="mt-1 text-xs text-muted-foreground">
                {m.monitor_form_max_retries_help()}
              </p>
            </div>

            <div>
              <label class="text-sm font-medium" for="monitor-resend-interval"
                >{m.monitor_form_resend_interval_label()}</label
              >
              <input
              ...
              />
              <p class="mt-1 text-xs text-muted-foreground">
                {m.monitor_form_resend_interval_help()}
              </p>
            </div>

            <div>
              <label class="text-sm font-medium" for="monitor-weight"
                >{m.monitor_form_weight_label()}</label
              >
              <input
                id="monitor-weight"
                type="number"
              ...
              />
              <p class="mt-1 text-xs text-muted-foreground">
                {m.monitor_form_weight_help()}
              </p>
            </div>

            <div>
              <label class="text-sm font-medium" for="monitor-group"
                >{m.monitor_form_group_label()}</label
              >
              <div class="mt-1">
                <Select
                  id="monitor-group"
                  options={[
                    ...(topLevelOptionAllowed
            ...
            <div>
              <label class="text-sm font-medium" for="monitor-proxy"
                >{m.monitor_form_proxy_label()}</label
              >
              <div class="mt-1">
                <Select
                  id="monitor-proxy"
            ...
            <div>
              <label class="text-sm font-medium" for="monitor-escalation"
                >{m.escalation_assign_label()}</label
              >
              <div class="mt-1">
                <Select
            ...
            <div class="flex items-center gap-2 pt-6">
              <input
                ...
                class="h-4 w-4 rounded border-border"
              />
              <label for="monitor-upside-down" class="text-sm font-medium"
                >{m.monitor_form_upside_down_label()}</label
              >
            </div>
              ...
              <div>
                <div class="flex items-center gap-2">
                  <input
                    ...
                    class="h-4 w-4 rounded border-border"
                  />
                  <label for="monitor-tls-ignore" class="text-sm font-medium"
                    >{m.monitor_form_tls_ignore_label()}</label
                  >
                </div>
            ...
            {#if selectedType === "http"}
              <div>
                <div class="flex items-center gap-2">
                  ...
                  />
                  <label
                    for="monitor-cert-expiry-notify"
                    class="text-sm font-medium"
