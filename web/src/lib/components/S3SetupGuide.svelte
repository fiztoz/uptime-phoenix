<script lang="ts">
	/**
	 * Modal that loads the S3 monitor setup guide (static markdown).
	 * Source of truth:
	 *   web/static/docs/s3-monitor-setup.md  →  /docs/s3-monitor-setup.md
	 *   docs/guides/s3-monitor-setup.md      (repo copy)
	 */
	...
	const GUIDE_PATH = "/docs/s3-monitor-setup.md";
	const GUIDE_FILENAME = "phoenix-s3-monitor-setup.md";
...
{#if open}
	...
	>
		...
		<div
			...
			role="dialog"
			aria-modal="true"
			aria-labelledby="s3-guide-title"
			tabindex="-1"
		>
			<div
			...
			<div class="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-5">
				{#if loading}
					<p class="text-sm text-muted-foreground">{m.s3_guide_loading()}</p>
				{:else if loadError}
					<div
					...
					>
						...
						<div>
							<p class="font-medium">{m.s3_guide_load_failed()}</p>
							<p class="mt-1 text-xs text-muted-foreground">{loadError}</p>
							<p class="mt-2 text-xs text-muted-foreground">
								{m.s3_guide_load_failed_hint()}
							</p>
						</div>
					</div>
				{:else if content}
