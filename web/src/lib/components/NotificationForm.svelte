<script lang="ts">
	...
	function parseDiscordButtons(config: Record<string, unknown>): DiscordLinkButton[] {
		...
		for (const item of raw) {
			...
			buttons.push({
				label: typeof row.label === 'string' ? row.label : typeof row.label_template === 'string' ? row.label_template : '',
				url: typeof row.url === 'string' ? row.url : typeof row.url_template === 'string' ? row.url_template : '',
			});
	...
	let discordButtons = $state<DiscordLinkButton[]>(
		parseDiscordButtons((initialNotification?.config || {}) as Record<string, unknown>),
	);
		...
		>
			...
			<div class="mt-6 space-y-4">
				...
				{#if selectedType === 'discord'}
					<div class="space-y-3 border-t border-border pt-4">
						<div class="flex items-start justify-between gap-3">
							<div>
								<p class="text-sm font-medium">{m.notification_form_discord_buttons_title()}</p>
								<p class="mt-1 text-xs text-muted-foreground">{m.notification_form_discord_buttons_help()}</p>
							</div>
							<button
								type="button"
								onclick={() => {
						...
						{#each discordButtons as button, index (`discord-button-${index}`)}
							<div class="grid gap-2 sm:grid-cols-[1fr_minmax(0,1.5fr)_auto]">
								<input
									...
									value={button.label}
									oninput={(e) => {
										discordButtons[index].label = (e.target as HTMLInputElement).value;
								...
								<input
									...
									value={button.url}
									oninput={(e) => {
										discordButtons[index].url = (e.target as HTMLInputElement).value;
										discordButtons = discordButtons;
									}}
									class={inputClass}
								...
								<button
									type="button"
									onclick={() => {
										discordButtons = discordButtons.filter((_, i) => i !== index);
									}}
				...
				<div class="flex flex-col gap-2 pt-2">
					...
					<div class="flex items-start gap-2">
						<input type="checkbox" bind:checked={formData.include_ack_url} id="include-ack-url" class="mt-0.5 h-4 w-4 rounded border-border accent-primary" />
						<label for="include-ack-url" class="text-sm text-muted-foreground">
							<span class="block">{m.notification_form_include_ack_label()}</span>
							<span class="block text-xs text-faint">
