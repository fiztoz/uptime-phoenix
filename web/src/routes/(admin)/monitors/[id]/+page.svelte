<script lang="ts">
	...
	import { realtime, type Heartbeat as WsHeartbeat } from '$lib/stores/ws.svelte.js';
	...
	async function runPostClearPoll(gen: number, attempt: number) {
		...
		if (gen !== postClearPollGeneration || !historyClearedAt) return;
		if (shouldStopPostClearPolling(timelineHeartbeats.length, statusHistory.length)) {
			stopPostClearPolling();
			return;
		}
	...
	$effect(() => {
		...
		if (lastKnownStatus !== null && lastKnownStatus !== newStatus) {
			...
			if (!statusHistory.some((h) => h.time === entry.time && h.status === entry.status)) {
				statusHistory = [entry, ...statusHistory].slice(0, 100);
			}
		} else if (statusHistory.length > 0 && statusHistory[0].status === newStatus) {
		...
		if (timelineHeartbeats.length === 0) {
			...
			const timelineTime = latestTimeline ? new Date(latestTimeline.time).getTime() : 0;
		...
		if (
			historyClearedAt &&
			shouldStopPostClearPolling(timelineHeartbeats.length, statusHistory.length)
		) {
			stopPostClearPolling();
		}
	...
	async function refreshObservabilityAfterClear() {
		const [history, timeline, statsData] = await Promise.all([
			heartbeatsApi
				.listOptions(monitorId, { hours: 720, limit: 100, order: 'desc', important: true })
				.catch(() => []),
			heartbeatsApi
		...
		const hint = deriveStatusHint(history[0], realtime.heartbeats.get(monitorId), statsData);
	...
	async function loadChartData(hours: number) {
		...
		chartLoading = true;
		const nextChart = await heartbeatsApi.chart(monitorId, hours).catch(() => null);
	...
	async function loadDetails() {
		...
		loadError = null;
		try {
			const [monitorData, statsData, history, chart, timeline] = await Promise.all([
	...
	async function assignNotification() {
		...
		try {
			await notificationsApi.assignToMonitor(monitorId, Number(selectedNotifToAdd));
	...
	async function assignTag() {
		...
		try {
			await tagsApi.assignToMonitor(monitorId, Number(selectedTagToAdd), tagValueInput.trim() || undefined);
	...
	async function unassignTag(tagId: number) {
		...
		const ok = await confirmAction({
			title: tag ? m.monitor_detail_page_remove_tag_named({ name: tag.name }) : m.monitor_detail_page_remove_tag_generic(),
	...
	function tagMeta(tagId: number): Tag | undefined {
		return allTags.find((t) => t.id === tagId) ?? assignedTags.find((mt) => mt.tag_id === tagId)?.tag;
	}
	...
	const isPaused = $derived(monitor?.status === 'paused' || monitor?.active === false);
	...
	async function togglePause() {
		...
		try {
			...
			const patched = monitorFromApi(
				updated,
				updated.active === false ? 'paused' : (monitor.status === 'paused' ? 'pending' : monitor.status),
			);
			fetchedMonitor = patched;
			realtime.patchMonitor(patched);
			toast.success(isPaused ? m.monitor_detail_page_resumed_toast() : m.monitor_detail_page_paused_toast());
		} catch (e: any) {
	...
	const avgPing24h = $derived(stats?.avg_ping_24h != null ? Math.round(stats.avg_ping_24h) : null);
	...
	const statusBadgeUrl = $derived(`${badgeOrigin}/api/badge/${monitorId}/status.svg`);
	const uptimeBadgeUrl = $derived(`${badgeOrigin}/api/badge/${monitorId}/uptime.svg?duration=24h`);
	const pingBadgeUrl = $derived(`${badgeOrigin}/api/badge/${monitorId}/ping.svg`);
...
<svelte:head>
	<title>{m.app_name()} · {monitor?.name ?? m.monitor_detail_page_title_fallback()}</title>
</svelte:head>
...
<div class="max-w-6xl space-y-6">
	...
	>
		<ArrowLeft class="h-4 w-4" /> {m.monitor_detail_page_back_to_monitors()}
	</button>

	{#snippet retryLoadAction()}
		<button type="button" onclick={loadDetails} class="inline-flex items-center gap-2 rounded-lg border border-border px-4 py-2 text-sm font-medium transition-colors hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{m.monitor_group_form_retry()}</button>
	{/snippet}
	...
	{:else if loadError}
		<EmptyState icon={AlertTriangle} title={m.monitor_detail_page_load_failed()} description={loadError} action={retryLoadAction} />
	{:else if !monitor}
		{#snippet notFoundAction()}
			<button type="button" onclick={goBack} class="inline-flex items-center gap-2 rounded-lg border border-border px-4 py-2 text-sm font-medium transition-colors hover:bg-accent">
				<ArrowLeft class="h-4 w-4" /> {m.monitor_detail_page_back_to_monitors_lower()}
			</button>
		{/snippet}
		<EmptyState icon={Activity} title={m.monitor_detail_page_not_found_title()} description={m.monitor_detail_page_not_found_description()} action={notFoundAction} />
	{:else}
		<!-- Header + actions -->
		<div
		...
		>
			<div class="min-w-0 flex-1">
				<h1 class="truncate text-2xl font-semibold tracking-tight">{monitor.name}</h1>
				<p class="mt-1 text-sm text-muted-foreground">
					<span class="font-mono text-xs">{monitor.type}</span>
					<span class="mx-1 text-faint">·</span>
					{m.monitor_detail_page_every_seconds({ seconds: monitor.interval || 60 })}
				</p>
				{#if targetUrl()}
					{@const url = targetUrl()}
					{@const href =
			...
			<div class="flex flex-wrap items-center gap-2" data-testid="monitor-toolbar">
				...
				<button
					...
					title={isPaused ? m.monitor_detail_page_resume() : m.monitor_detail_page_pause()}
				>
					{#if isPaused}<Play class="h-4 w-4" />{:else}<Pause class="h-4 w-4" />{/if}
					{isPaused ? m.monitor_detail_page_resume() : m.monitor_detail_page_pause()}
				</button>
				<button
					type="button"
				...
				>
					<Pencil class="h-4 w-4" /> {m.btn_edit()}
				</button>
				<button
					type="button"
					onclick={handleClone}
					...
					class="inline-flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-sm transition-colors hover:bg-accent disabled:opacity-50"
				>
					<Copy class="h-4 w-4" /> {m.monitor_detail_page_clone()}
				</button>
				<button
					type="button"
				...
				>
					<Trash2 class="h-4 w-4" /> {m.btn_delete()}
				</button>
			</div>
		</div>
		...
		>
			...
			/>
			<MetricCard label={m.monitor_detail_page_metric_uptime_24h()} value={uptime24h} />
			<MetricCard label={m.monitor_detail_page_metric_uptime_30d()} value={uptime30d} />
			{#if showCert}
				<MetricCard
					label={m.monitor_detail_page_metric_cert_exp()}
					value={certExpiry() ? certExpiry()!.date : '—'}
					unit={certExpiry() ? m.monitor_detail_page_days_unit({ days: certExpiry()!.days }) : undefined}
				/>
			{/if}
		</div>
		...
		<div class="grid gap-4 md:grid-cols-2">
			<div class="rounded-xl border border-border bg-card p-5">
				<h3 class="flex items-center gap-2 text-sm font-semibold tracking-tight">
					<Activity class="h-4 w-4 text-muted-foreground" /> {m.monitor_detail_config()}
				</h3>
				<div class="mt-4 space-y-2 text-sm">
					<div class="flex items-center justify-between gap-4">
						<span class="shrink-0 text-muted-foreground">{m.monitors_target()}</span>
						<span class="break-all text-right font-mono text-xs"
							>{targetUrl() || JSON.stringify(monitor.config).slice(0, 60)}</span
						>
					</div>
					<div class="flex items-center justify-between border-t border-border pt-2">
						<span class="text-muted-foreground">{m.monitor_detail_page_created_label()}</span>
						<span class="tnum">{new Date(monitor.created_at).toLocaleDateString()}</span>
					</div>
				</div>
			</div>
			...
			<div class="rounded-xl border border-border bg-card p-5">
				<div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
					<h3 class="flex items-center gap-2 text-sm font-semibold tracking-tight">
						<TagIcon class="h-4 w-4 text-muted-foreground" /> {m.monitor_detail_tags()}
					</h3>
				...
				{#if assignedTags.length === 0}
					<p class="mt-4 text-sm text-muted-foreground">{m.monitor_detail_no_tags()}</p>
				{:else}
					<div class="mt-4 flex flex-wrap gap-2">
						{#each assignedTags as mt (mt.tag_id)}
							...
							>
								{meta?.name ?? m.monitor_detail_page_tag_fallback({ id: mt.tag_id })}
								{#if mt.value}
		...
		<div class="rounded-xl border border-border bg-card p-5">
			<h3 class="flex items-center gap-2 text-sm font-semibold tracking-tight">
				<BadgeCheck class="h-4 w-4 text-muted-foreground" /> {m.monitor_detail_page_status_badge_heading()}
			</h3>
			<p class="mt-1 text-sm text-muted-foreground">
			...
			<div class="mt-4 flex flex-wrap items-center gap-4">
				<img src={statusBadgeUrl} alt={m.monitor_detail_page_badge_alt_status({ name: monitor.name })} class="h-5" />
				<img src={uptimeBadgeUrl} alt={m.monitor_detail_page_badge_alt_uptime({ name: monitor.name })} class="h-5" />
				<img src={pingBadgeUrl} alt={m.monitor_detail_page_badge_alt_ping({ name: monitor.name })} class="h-5" />
			</div>
			...
			<div class="mt-4 space-y-2">
				{#each [{ label: m.monitor_detail_page_badge_status(), url: statusBadgeUrl }, { label: m.monitor_detail_page_badge_uptime_24h(), url: uptimeBadgeUrl }, { label: m.monitor_detail_page_badge_ping(), url: pingBadgeUrl }] as badge (badge.label)}
					<div class="flex items-center gap-2">
						<span class="w-24 shrink-0 text-xs text-muted-foreground">{badge.label}</span>
						<code
						...
						<button
							type="button"
							onclick={() => copyToClipboard(badge.url, m.monitor_detail_page_badge_url_label({ label: badge.label }))}
						...
						>
							<Copy class="h-3.5 w-3.5" /> {m.monitor_detail_page_copy()}
						</button>
					</div>
				{/each}

				<div class="flex items-start gap-2 border-t border-border pt-3">
					<span class="w-24 shrink-0 pt-1.5 text-xs text-muted-foreground">{m.monitor_detail_page_markdown_label()}</span>
					<code
					...
					<button
						type="button"
						onclick={() => copyToClipboard(badgeMarkdownSnippet, m.monitor_detail_page_markdown_snippet())}
						class="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-border px-2.5 py-1.5 text-xs transition-colors hover:bg-accent"
					>
						<ClipboardCopy class="h-3.5 w-3.5" /> {m.monitor_detail_page_copy()}
					</button>
				</div>
			</div>
		</div>
		...
		<div class="rounded-xl border border-border bg-card p-5">
			<div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
				<h3 class="flex items-center gap-2 text-sm font-semibold tracking-tight">
					<Bell class="h-4 w-4 text-muted-foreground" /> {m.monitor_detail_notifications()}
				</h3>
				<div class="flex flex-wrap gap-2">
					<div class="flex-1 sm:w-auto">
						<Select
							options={[
								...
								...allNotifications
									.filter((n) => !assignedNotifications.some((a) => a.id === n.id))
									.map((n) => ({ value: String(n.id), label: `${n.name} (${n.type})` })),
							]}
			...
			{#if assignedNotifications.length === 0}
				<p class="mt-4 text-sm text-muted-foreground">{m.monitor_detail_no_notifications()}</p>
			{:else}
