...
1. M0 wire/API/browser fixtures, atomic commit/ingest ports, monitor-create local assignment, hub scheduler ownership, and heartbeat `probe_id`/rollup unique `(monitor_id,probe_id,bucket)` are in place. Do not advertise full `phoenix.probe.v1` capability. Remaining M1 work is overall readers and incident/delivery persistence.
...
4. Integrate shared retry/condition/maintenance evaluation with regional commit. Prove T01/T02/T24/T31/T33 against real persistence, not just the current pure-policy tests. T03 (rollup unique-key replacement with preserved IDs) is covered by `037_probe_heartbeat`. Connect overall readers and update all UNKNOWN consumers before emitting the new status.
