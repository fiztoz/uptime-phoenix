...
| Retry | Pure `EvaluateRetry`/`EvaluateObservation` reused by `HeartbeatService.Record`; maintenance then retry; independent state inputs; local Record commits observation+state for the `local` assignment; `PromoteCondition` is the shared consecutive/hysteresis rule | Do not dispatch regional incidents from the existing dispatcher; capacity and certificate state now use assignment-specific repository views; durable regional delivery remains open |
...
**New follow-up from code inspection:** `HeartbeatService.Record` currently derives local sequence from each monitor's state, while every local monitor uses `domain.LocalStreamID` and `probe_observations` enforces global `UNIQUE(stream_id, seq)`. Different monitors can therefore attempt the same sequence. Before expanding the local-parity claim, add a two-monitor/concurrent-writer regression and a single durable stream allocator tied to recording. Preserve sequence across restart/removal, and avoid claiming atomic heartbeat/regional/auxiliary writes while they are still separate operations. The auxiliary integration test in this slice covers one monitor across assignment generations, not that multi-monitor stream contract. Prioritize this before the remaining dispatcher work.
...
2. Recheck HEAD before reserving numbers after `041_probe_auxiliary_state`. Heartbeat partition expression and rollup auto-increment IDs are preserved; regional readers still mix monitor-wide heartbeat history until they are scoped.
