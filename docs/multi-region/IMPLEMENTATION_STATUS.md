...
| Protocol | Bounded envelope validation, all five V1 telemetry kinds in mixed batches, ACK/retry/gap payload validation, complete current-state DTOs and four transfer frames, hash-checked bounded staging, hello/welcome/health DTOs and trusted handshake comparison, 165 valid/invalid fixtures | Config/commands/enrollment, authenticated session/lease integration, durable cursor/assignment/incident/channel validation, atomic state application/receipts and complete API/browser fixtures |
...
1. Finish the M0 contract gaps listed in `PROTOCOL.md` section 10: complete configuration DTOs/transfers and reference validation, enrollment/commands, API/browser shapes, and complete fixture index. Handshake/health DTOs, current-state schemas/chunks, and all telemetry payload schemas are covered; authenticated session integration, durable application, and identity correlation remain open. Do not advertise full `phoenix.probe.v1` capability on the strength of these decoders.
