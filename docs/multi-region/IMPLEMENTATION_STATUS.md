...
|---|---|---|
| Domain | Reserved `local` identity, registration/assignment types, positive revision/generation contracts, `UNKNOWN=4`, ANY/ALL policy and count/coverage types, regional observation/state/stream/incident/delivery/command types, overall projection/history/dirty-bucket types | Persisted assignment effective-time history |
...
1. M0 wire/API/browser fixtures, atomic commit/ingest ports, monitor-create local assignment, hub scheduler ownership, heartbeat `probe_id`/rollup unique `(monitor_id,probe_id,bucket)`, overall health readers, incident/delivery persistence, local `RegionalCommit`, shared retry/maintenance/condition evaluation, and materialized overall projections/dirty-bucket history are in place. Do not advertise full `phoenix.probe.v1` capability. Remaining M1 work is scoped alert throttling/certificate/capacity/escalation and paired edge-schema migrations.
2. Recheck HEAD before reserving numbers after `039_probe_health_projection`. Heartbeat partition expression and rollup auto-increment IDs are preserved; regional readers still mix monitor-wide heartbeat history until they are scoped.
...
4. Prove T01/T02/T24/T31/T33 against real persistence. T03 is covered by `037_probe_heartbeat`. Current overall ANY/ALL readers and historical overall reconstruction exist; update all UNKNOWN consumers before emitting the new status on existing routes. Incident/delivery rows are stored but not wired into the existing dispatcher. Persist assignment effective-time history before relying on remove/re-add reconstruction.
