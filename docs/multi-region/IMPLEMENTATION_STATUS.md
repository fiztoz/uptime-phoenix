...
Monitor create and hub scheduling now honor local assignment ownership. Existing local heartbeat writes are unchanged. `phoenix.probe.v1` is not advertised as complete. No probe HTTP mutation routes were added.
...
1. M0 wire/API/browser fixtures, atomic commit/ingest ports, monitor-create local assignment, and hub scheduler ownership are in place. Do not advertise full `phoenix.probe.v1` capability. Remaining M1 work is heartbeat `probe_id`/rollup unique-key replacement, overall readers, and incident/delivery persistence.
2. Recheck HEAD before reserving numbers after `036_probe_regional`. Preserve partition-safe heartbeat identity and rollup auto-increment keys until readers are scoped.
