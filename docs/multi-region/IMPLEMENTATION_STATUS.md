...
|---|---|---|
| Domain | Reserved `local` identity, registration/assignment types, positive revision/generation contracts, `UNKNOWN=4`, ANY/ALL policy and count/coverage types, regional observation/state/stream/incident/delivery/command types | Projection types used by historical overall readers |
| Retry | Pure `EvaluateRetry` reused by the existing `HeartbeatService.Record`; independent state inputs, retry confirmation, maintenance/raw-PENDING reset and overflow saturation; local Record commits observation+state for the `local` assignment | Extract condition/maintenance evaluation; do not dispatch regional incidents from the existing dispatcher |
...
| Database | Migrations `035`–`038` on MariaDB/SQLite; local backfill; credential-free registration stores; atomic revision-checked assignment replacement; tombstones prevent generation reuse; `RegionalCommit`/`Ingest` persist per-probe observations, cursors, and optional incidents; delivery outcomes correlate to stored transitions; monitor Create writes a local assignment in the same transaction; hub ClaimBatch/schedulers skip remote-only sets; heartbeats carry `probe_id` (default `local`) and rollups unique `(monitor_id,probe_id,bucket)` | Historical overall projections, edge DB, config snapshot storage |
...
1. M0 wire/API/browser fixtures, atomic commit/ingest ports, monitor-create local assignment, hub scheduler ownership, heartbeat `probe_id`/rollup unique `(monitor_id,probe_id,bucket)`, overall health readers, incident/delivery persistence, and local `RegionalCommit` from `HeartbeatService.Record` are in place. Do not advertise full `phoenix.probe.v1` capability. Remaining M1 work is historical overall projections and condition/maintenance extraction.
2. Recheck HEAD before reserving numbers after `038_probe_incidents`. Heartbeat partition expression and rollup auto-increment IDs are preserved; regional readers still mix monitor-wide heartbeat history until they are scoped.
...
4. Extract condition/maintenance evaluation into the same recorder path. Prove T01/T02/T24/T31/T33 against real persistence. T03 is covered by `037_probe_heartbeat`. Current overall ANY/ALL readers exist; update all UNKNOWN consumers before emitting the new status on existing routes. Incident/delivery rows are stored but not wired into the existing dispatcher.
