...
| Protocol | Bounded envelope validation, all five telemetry kinds, ACK/retry/gap, complete state/config DTOs and transfer frames, bounded hash-checked staging, config reference/target/capability checks, revision comparison, hello/welcome/health and trusted handshake comparison, 225 valid/invalid fixtures | Commands/enrollment and API/browser contracts; config building, extension/runtime validators and atomic activation; authenticated sessions/leases, durable ingest authorization/cursors and state application/receipts |
...
1. Finish the M0 contract gaps listed in `PROTOCOL.md` section 10: enrollment/commands and rotation/reset contracts, exact API/browser shapes, pure regional/stream/incident/command types and atomic ports, and complete baseline/runtime-extension fixtures. Handshake/health, current-state, config schemas/transfers/reference checks, and all telemetry payload schemas are covered. Authenticated sessions, config building and extension validation, atomic activation/application, and durable identity correlation remain open. Do not advertise full `phoenix.probe.v1` capability on the strength of these decoders.
