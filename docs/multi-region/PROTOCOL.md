...
capability; see [watchdog acceptance](M3_WATCHDOG_ACCEPTANCE.md). Regional ACK
transitions now require durable issued-command correlation; uncorrelated and
watchdog ACK transitions remain rejected. See [command acceptance](M3_COMMAND_ACCEPTANCE.md).
...
`telemetry.ack.payload` contains `stream_id`, `committed_seq`, `accepted_count`, `duplicate_count`, and `rejected` (array of permanently rejected `seq`, `code`). The cursor represents every preceding sequence either durably accepted, durably rejected with a receipt/reason, or covered by an explicit persisted gap. The ordered replay implementation acknowledges through the request's `last_seq`, never beyond the batch the edge sent, even when the hub already retains a later prefix. This is a durable prefix proof, not permission to prune unsent rows. `accepted_count` counts newly accepted events; `duplicate_count` counts previously accepted events; `rejected` includes both new and repeated permanent rejections. These outcomes partition the request without double-counting rejected duplicates.
...
The current ordered replay increment conservatively closes the session on any ingest error after rollback. The edge retains its local cursor and reconnects to replay the same prefix. The edge can consume `telemetry.retry`, but hub emission of that frame for recoverable storage failures remains unimplemented; the increment does not claim the complete error-reporting behavior above.
...
The implemented credential source uses an absolute overlap deadline at most ten
minutes after preparation command creation, with microsecond timestamps. Retries
never extend it. A candidate may authenticate during preparation; the original
credential remains active if activation never commits. After activation the new
credential remains active indefinitely, while the previous credential expires at
the original deadline. Observed expiry is durably closed and cannot revive after
a backward clock step. A newer rotation must exceed the durable version high-water
and wait until the prior overlap is closed or expired.

Socket admission atomically rechecks credential eligibility and commits its new
connection generation. Before activation, both admitted tokens carry the overlap
session deadline; after activation only the previous token carries that deadline.
Successful prepare/activate and recovered success receipts quiesce further
non-health effects after the result write. The hub closes after committing the
receipt; the source forces closure within 12 seconds if needed, still honoring
its admitted credential deadline. This permits the hub's bounded receipt commit
without treating socket close as success. Reconnection rechecks credential
eligibility and generation authority. Source migration 007 stores only token digests, at most 1,024
retained rotations, and the existing shared 16,384-entry command receipt bound.
Hub migration 062 protects the candidate and both command requests before dispatch.
Capability-aware claiming releases activation only after an exact prepare result.
Candidate authentication alone never promotes the current credential; only the
original durable activation result does. Pending candidate selection survives
expiry/restart for lost-result recovery, with current-credential fallback limited
to pinned HTTP 401 before websocket admission. See
[hub rotation acceptance](M3_HUB_CREDENTIAL_ACCEPTANCE.md).
