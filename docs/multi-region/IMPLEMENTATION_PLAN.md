...
- [ ] Define transport DTOs independently from domain types. Create valid/invalid fixtures under `internal/adapters/probe/testdata/v1/` and browser response fixtures in the existing frontend test layout.
  - Current-state DTOs, all four state transfer frames, bounded assembly, and all five telemetry event kinds are implemented, with 112 valid/invalid fixtures in total. Complete configuration, handshake/commands/enrollment, and API/browser fixtures remain open.
