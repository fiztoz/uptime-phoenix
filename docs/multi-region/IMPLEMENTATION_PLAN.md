...
- [x] Add `local` registration, monitor assignments/generations, regional state, stream cursor/receipt tables, scoped incident/delivery storage, config/command metadata, projection/history, and dirty-bucket schema.
  - Registration/assignments (`035`), regional observations/state/streams/commands/dirty buckets (`036`), incident/delivery mirrors (`038`), overall health projections (`039`), assignment/policy history (`040`), capacity/TLS assignment identity (`041`), local stream allocation (`042`), availability attempt throttles (`043`), alert assignment identity (`044`), source availability delivery work (`045`), legacy source identity/version (`046`), and encrypted prepared snapshots (`047`) are in both engines. Escalation inherits assignment identity through its immutable alert ID. Dedicated edge migrations 001–002 now ship with M2.
- [x] Add paired MariaDB/SQLite migrations and edge-schema migrations. Preserve existing heartbeat partition expression and IDs. Keep rollup auto-increment IDs; replace the old unique `(monitor_id,bucket)` key correctly.
  - Hub `036` regional tables, `037` heartbeat `probe_id` / rollup unique `(monitor_id,probe_id,bucket)`, `038` incident/delivery tables, `039` overall health projections, `040` assignment history, `041` capacity/TLS state, `042` local stream allocation, `043` availability attempt throttles, `044` scoped alerts, `045` source delivery intents, `046` legacy source identity/version, and `047` encrypted prepared snapshots are paired. Dedicated edge migrations 001–002 now ship with M2.
- [x] Implement atomic regional commit and ingest transaction ports with real-engine tests; use a persisted sequence and state transaction on SQLite.
  - `RegionalCommitStore` commits observation+state together and ingest is idempotent with gap rejection. The `042` local path atomically allocates a shared stream sequence and writes heartbeat+observation+state+dirty buckets, with stale-evaluation retry. Migration `045` extends both source recording ports with optional incident transitions and availability intents, plus scoped attempt leases and atomic outcome completion. Migration `046` supplies stable source UUIDs and atomic lifecycle versions for legacy alerts, preserving acknowledgement links. The key-configured local recorder now atomically plans lifecycle/initial intents/escalation registration, with versioned applied configuration and an integrated outbox consumer. Authenticated remote ingest remains M3.
- [x] Refactor shared retry/maintenance/condition evaluation into a reusable service operation. Keep the local scheduler routed through a `local` assignment.
  - `EvaluateObservation` applies maintenance then retry. `PromoteCondition` is the pure consecutive/hysteresis rule. `HeartbeatService.Record` uses both and commits local regional state. Configured local availability uses the durable source outbox; group/status-page effects retain their dispatcher owner.
...
- [x] Scope alert throttling, certificate thresholds, capacity promotion, and escalation state. Preserve current single-local-monitor alert behavior.
  - `041` scopes capacity counters/cursors and certificate thresholds by probe and assignment generation. Local `Record` uses bound repository views; existing dashboard reads select current-local evidence. `043` persists availability attempt throttles with atomic resend reservations and local dispatcher wiring. `044` scopes alert lifecycle by probe/generation and preserves escalation ownership through the alert ID. The dispatcher binds its heartbeat generation; the hub escalation runner claims local incidents and checks active generations before sending. Migration `045` and the integrated local runtime now cover lifecycle planning, versioned applied channels, obsolete-intent reconciliation and durable sends. Migration `051` retains escalation policy/step context. M2 adds direct edge availability delivery; remote auxiliary alerts and commands remain later milestones.
...
- [x] Add `cmd/probe/main.go`, edge bootstrap/config validation, dedicated SQLite migrations, process/data-directory lock, and stable TLS/identity files.
- [x] Reuse current checker/notifier implementations. Enable and advertise HTTP/TCP/DNS for the engineering slice; reject unsupported assignments.
- [x] Implement pinned TLS 1.3, required fingerprint validation, header credentials, locally generated enrollment token, crash-safe credential exchange, version negotiation, and endpoint policy.
- [x] Implement a bounded one-reader/one-writer session supervisor, cancellation, limits, deadlines, fair control/replay scheduling, and backoff with jitter.
- [x] Implement hub connector ownership with DB lease and fencing. Test old-session close callbacks and duplicate running probe identity.
- [x] Implement direct regional delivery from a durable local outbox. Recording does not block on provider I/O.
- [x] Add health/readiness diagnostics that remain meaningful during hub outage.
...
**Verified 2026-09-20:** acceptance, commit references, commands, process evidence and remaining M3 boundaries are in [M2_ACCEPTANCE_REPORT.md](M2_ACCEPTANCE_REPORT.md). The local parity corrections are in the linked retrospective. This supersedes older foundation-only notes above.
