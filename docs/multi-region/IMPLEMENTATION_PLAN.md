...
- [ ] Define transport DTOs independently from domain types. Create valid/invalid fixtures under `internal/adapters/probe/testdata/v1/` and browser response fixtures in the existing frontend test layout.
  - Current-state DTOs, all four state transfer frames, bounded assembly, all five telemetry event kinds, and hello/welcome/health DTOs with trusted handshake comparison are implemented, with 165 valid/invalid fixtures in total. Complete configuration, commands/enrollment, and API/browser fixtures remain open; authenticated session/lease integration is not supplied by the handshake helper.
