...
- [ ] Add paired MariaDB/SQLite migrations and edge-schema migrations. Preserve existing heartbeat partition expression and IDs. Keep rollup auto-increment IDs; replace the old unique `(monitor_id,bucket)` key correctly.
  - Hub `036` regional tables, `037` heartbeat `probe_id` / rollup unique `(monitor_id,probe_id,bucket)`, `038` incident/delivery tables, `039` overall health projections, `040` assignment history, `041` capacity/TLS state, `042` local stream allocation, `043` availability attempt throttles, `044` scoped alerts, `045` source delivery intents, `046` legacy source identity/version, and `047` encrypted prepared snapshots are paired. Edge schema remains M2.
...
- [ ] Build complete per-probe snapshots from authorized assignments and dependencies. Changes persist desired revision and durable sync work.
  - `047` implements bounded complete-document inspection and protected immutable preparation on both hub databases, including an internal local decoder. The local builder captures one consistent database view, resolves the selected dependency graph, encodes deterministic complete documents, and prepares protected revisions. Maintenance follows persisted links; empty links mean no suppression. Exact local prepared revisions can now be validated against installed checker/notifier validators, shared template rules, cron/timezones and proxy syntax without executing checks or sends. Explicit no-replace key provisioning and protected file loading are implemented, with [backup/recovery instructions](KEY_PROVISIONING.md). Runtime key wiring, retained-snapshot authentication at startup, remote construction/validation, environment readiness, assignment/session authority, desired sync work, atomic activation and applied receipts remain open.
