# Uptime Kuma → Uptime Phoenix importer

Convert a **stopped** Uptime Kuma snapshot into a Uptime Phoenix `BackupDocument`
...
database, never writes a Uptime Phoenix database, and never calls a live Uptime Phoenix API.
...
Treat it the same way as a Uptime Phoenix backup export (see `docs/RUNBOOK.md` §1):
...
2. **Snapshot the database**:
   ...
   - Docker example:
     ...
     ```
3. Keep a full backup of Kuma until you have verified the Uptime Phoenix import.
...
4. Keep the dump until Uptime Phoenix import is verified.
...
| `--output` | yes | Uptime Phoenix backup JSON path (created mode `0600`) |
...
### Monitors (Kuma type → Uptime Phoenix type)

| Kuma type | Uptime Phoenix type | Notes |
|---|---|---|
...
| `systemd` / `system-service` | — | **skipped** (deferred until Uptime Phoenix agent) |
| `gamedig` / `steam` | — | **skipped** (excluded from Uptime Phoenix) |
| `tailscale` | — | **skipped** (excluded from Uptime Phoenix) |
| `radius` | — | **skipped** (excluded from Uptime Phoenix) |
...
### Notifications (Kuma type → Uptime Phoenix provider)

Only Uptime Phoenix's locked **11** providers are imported. Config keys are remapped from
Kuma camelCase into Uptime Phoenix snake_case (for example `telegramBotToken` →
`bot_token`).

| Kuma type(s) | Uptime Phoenix type |
|---|---|
| `telegram` | `telegram` |
| `discord` | `discord` |
| `slack` | `slack` |
...
| Status-page email subscribers | **Never imported** (re-subscribe in Uptime Phoenix) |

Unsupported entities are reported with source ID, type, and reason. Nothing is
silently coerced into a different Uptime Phoenix type.

## Admin import step

After conversion:

1. Sign in to Uptime Phoenix as an **admin** (session JWT).
2. Import the JSON with the existing backup import endpoint (same as a Uptime Phoenix
...
3. Import is **merge-only**: it creates new entities and remaps source IDs. It
   does not overwrite or delete existing Uptime Phoenix data.
...
column is absent, related fields are left at Uptime Phoenix defaults. Reserved table
names such as `group` are quoted per dialect (`"group"` vs `` `group` ``).

## What this is not

- Not a live sync or continuous migration
- Not an in-place writer into Kuma or Uptime Phoenix SQLite/MariaDB
- Not a full disaster-recovery tool (heartbeats and history are intentionally
  left behind — start fresh checks after import)
- Not a subscriber migration (re-collect status-page email subscribers in Uptime Phoenix)
...
read-only DSN (never use the production Uptime Phoenix `phoenix` database).
