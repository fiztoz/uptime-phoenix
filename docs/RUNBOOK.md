...
| `OIDC_CAP_*_GROUPS` | empty | IdP groups for capability flags (`NOTIFICATIONS`, `MAINTENANCE`, `CREATE_MONITORS`, `CREATE_TOP_LEVEL_MONITORS`, `CREATE_GROUPS`, `EDIT_GROUP_METADATA`, `VIEW_EXTENSIONS`, `VIEW_ALL_MONITORS`). `VIEW_ALL_MONITORS` is read-only visibility of every monitor/group; it does not grant write. |
