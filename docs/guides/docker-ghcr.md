...
| Image | Use |
|---|---|
| `ghcr.io/fiztoz/uptime-phoenix:0.2.3` | All-in-one (API + worker + embedded UI) |
| `ghcr.io/fiztoz/uptime-phoenix:latest` | Same, floating tag (prefer pin for production) |
| `ghcr.io/fiztoz/uptime-phoenix-api:0.2.3` | API tier only |
| `ghcr.io/fiztoz/uptime-phoenix-worker:0.2.3` | Worker tier only |
| `ghcr.io/fiztoz/uptime-phoenix-web:0.2.3` | Static UI (nginx) for split web tier |
...
# Split images
docker pull ghcr.io/fiztoz/uptime-phoenix-api:0.2.3
docker pull ghcr.io/fiztoz/uptime-phoenix-worker:0.2.3
docker pull ghcr.io/fiztoz/uptime-phoenix-web:0.2.3
```
...
docker run -d --name uptime-phoenix \
  ...
  ghcr.io/fiztoz/uptime-phoenix:0.2.3
...
services:
  uptime-phoenix:
    image: ghcr.io/fiztoz/uptime-phoenix:0.2.3
    ports:
      - "3000:3000"
    environment:
...
services:
  ...
  api:
    image: ghcr.io/fiztoz/uptime-phoenix-api:0.2.3
    depends_on:
      mariadb:
  ...
  worker:
    image: ghcr.io/fiztoz/uptime-phoenix-worker:0.2.3
    depends_on:
      api:
        condition: service_healthy
    environment:
...
Optional separate web image (nginx SPA) is `ghcr.io/fiztoz/uptime-phoenix-web:0.2.3` —
...
docker pull ghcr.io/fiztoz/uptime-phoenix:0.2.4
docker stop uptime-phoenix && docker rm uptime-phoenix
# re-run with the new tag (keep the same volume for SQLite)
```

Prefer **version tags** (`0.2.3`) over `latest` for reproducible deploys.
