...
| Image | Use |
|---|---|
| `ghcr.io/fiztoz/uptime-phoenix:0.3.1` | All-in-one (API + worker + embedded UI) |
| `ghcr.io/fiztoz/uptime-phoenix:latest` | Same, floating tag (prefer pin for production) |
| `ghcr.io/fiztoz/uptime-phoenix-api:0.3.1` | API tier only |
| `ghcr.io/fiztoz/uptime-phoenix-worker:0.3.1` | Worker tier only |
| `ghcr.io/fiztoz/uptime-phoenix-web:0.3.1` | Static UI (nginx) for split web tier |
...
# Split images
docker pull ghcr.io/fiztoz/uptime-phoenix-api:0.3.1
docker pull ghcr.io/fiztoz/uptime-phoenix-worker:0.3.1
docker pull ghcr.io/fiztoz/uptime-phoenix-web:0.3.1
```
...
docker run -d --name uptime-phoenix \
  ...
  ghcr.io/fiztoz/uptime-phoenix:0.3.1
...
services:
  uptime-phoenix:
    image: ghcr.io/fiztoz/uptime-phoenix:0.3.1
    ports:
      - "3000:3000"
    environment:
...
services:
  ...
  api:
    image: ghcr.io/fiztoz/uptime-phoenix-api:0.3.1
    depends_on:
      mariadb:
  ...
  worker:
    image: ghcr.io/fiztoz/uptime-phoenix-worker:0.3.1
    depends_on:
      api:
        condition: service_healthy
    environment:
...
Optional separate web image (nginx SPA) is `ghcr.io/fiztoz/uptime-phoenix-web:0.3.1` —
...
Prefer **version tags** (`0.3.1`) over `latest` for reproducible deploys.
