...
// EscalationPolicyRepo implements ports.EscalationPolicyRepository on MariaDB.
type EscalationPolicyRepo struct{ db *bun.DB }

// NewEscalationPolicyRepo creates a MariaDB-backed escalation policy repository.
...
func (r *EscalationPolicyRepo) Update(ctx context.Context, p *domain.EscalationPolicy) error {
	...
	err := r.db.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		...
		if n, err := res.RowsAffected(); err == nil && n == 0 {
			// Distinguish "no such policy" from "nothing changed": MariaDB reports
...
// EscalationAssignmentRepo implements ports.EscalationAssignmentRepository on MariaDB.
type EscalationAssignmentRepo struct{ db *bun.DB }

// NewEscalationAssignmentRepo creates a MariaDB-backed assignment repository.
...
// AlertEscalationRepo implements ports.AlertEscalationRepository on MariaDB.
type AlertEscalationRepo struct{ db *bun.DB }

// NewAlertEscalationRepo creates a MariaDB-backed alert escalation repository.
