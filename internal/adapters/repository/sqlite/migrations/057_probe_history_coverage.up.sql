...
INSERT OR IGNORE INTO probe_dirty_buckets (monitor_id,probe_id,resolution,bucket) SELECT monitor_id,'local','overall',bucket FROM probe_dirty_buckets WHERE resolution = 'overall' GROUP BY monitor_id,bucket;
DELETE FROM probe_dirty_buckets WHERE resolution = 'overall' AND probe_id <> 'local';
UPDATE probe_dirty_buckets SET revision = lower(hex(randomblob(16)));
