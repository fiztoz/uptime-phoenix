...
func TestSQLiteMigrationRebuildRollsBackWithChildren(t *testing.T) {
	...
	for _, stmt := range []string{
		...
		"INSERT INTO alerts (id, monitor_id, status, message, fired_at, ack_token, open_monitor_id) VALUES (1, 1, 'firing', 'down', CURRENT_TIMESTAMP, 'opaque-test-token', 1)",
	...
	source, err := sqliteMigrations.ReadFile("sqlite/migrations/044_probe_alert_scope.down.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	var probe, token string
	if err := db.QueryRowContext(ctx, "SELECT probe_id, ack_token FROM alerts WHERE id = 1").Scan(&probe, &token); err != nil || probe != "local" || token != "opaque-test-token" {
